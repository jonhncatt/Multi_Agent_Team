...
OpenAI native SDK、Responses API 和 streaming 会在后续作为独立 adapter work 继续推进，而不是成为这个稳定发布的默认 runtime path。v2.9.10 修复模型一次返回多个 tool calls 时的协议闭合问题：harness 会 drain 所有工具调用，并为每个 `tool_call_id` 写入且只写入一个 ToolMessage 后，才允许下一次模型请求。
