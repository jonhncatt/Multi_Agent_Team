...
v2.9.11 は Codex-style path portability と large-folder search safety の修正リリースであり、v2.9.0 の安定回復方針を維持します。
...
OpenAI native SDK、Responses API、streaming は今後の adapter work として分離し、この安定版の既定 runtime path には入れません。v2.9.11 は v2.9.10 の Codex-style all-tool drain semantics を維持し、model-visible な file path を project-relative path 優先にします。大きなフォルダで広すぎる glob search を行った場合は、大量の absolute path を model context に流し込まず、検索を絞り込む guidance を返します。
...
`exec_command` は引き続き保守的な allowlist を使います。v2.9.11 の推奨完全安全リストには `printf` と `dir` が含まれ、`VP_ALLOWED_COMMANDS` は追記ではなく完全上書きです。`rm`、`chmod`、`chown`、`curl`、`wget`、`sudo`、`dd`、`kill`、`pkill`、`brew`、`pip`、`pip3` などの高リスクコマンドは引き続きブロックされます。
