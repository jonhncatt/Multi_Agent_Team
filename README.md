...
OpenAI native SDK、Responses API 和 streaming 会作为后续单独的 adapter work（适配器工作）继续推进，而不是进入这个稳定发布的默认路径。v2.9.4 只清理 `/api/runtime-status` 的重复 provider/auth/descriptor/context meter 计算，不改变稳定 LangChain runtime 的行为。
