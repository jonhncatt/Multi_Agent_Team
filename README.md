...
OpenAI native SDK、Responses API 和 streaming 会作为后续单独的 adapter work（适配器工作）继续推进，而不是进入这个稳定发布的默认路径。v2.9.7 移除旧 proposal 执行链路，将工具调用明确视为模型行动，由 harness 统一验证 RuntimeBoundary、schema 和权限；无效工具调用会作为观察结果返回给模型自我修正。
