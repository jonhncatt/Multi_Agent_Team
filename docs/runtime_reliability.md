...
- GPT-5.4 and GPT-5.6 use a 272,000-token default operational window, a 90% automatic compaction threshold, and a 95% effective hard limit. GPT-5.6's 1,050,000-token model maximum is tracked separately. A verified company deployment may override the operational window with `VA_CONTEXT_WINDOW_TOKENS` or the threshold with `VA_CONTEXT_AUTO_COMPACT_TOKEN_LIMIT`.
