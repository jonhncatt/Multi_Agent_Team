...
| 2 | 真实任务 Eval | 测量对话、文件分析和代码生成的真实成功率 | `实施中` |
...
- 不再把文件数量和总体大小统计作为开始 Eval 的前置条件；规模分层留到出现真实性能或上下文问题时再补；
- 第一条当前架构 Eval 使用 `.cpp` 文件，但实现风格以过程式 C 子集为主；
- 案例采用协议帧解析，要求同时阅读 Markdown 规格、规则、公开头文件和参考实现；
- 禁止类、模板、STL、异常和动态内存，只允许修改目标 `.cpp`；
- 每次尝试在独立工作区运行，正式公司基线重复三次；
- 公司专用编译器通过本机包装脚本接入，包装脚本可以继续调用远程编译服务器；
- 先建立未调优的基线，再根据上下文读取、代码正确性、验证行为和完成状态分类结果调整 Agent；Eval 与 Agent 调优不放在同一次修改中。

当前实施入口：

```text
scripts/run_evals.py
evals/agent_quality_cases.json
```

本机实现验证已经完成：suite/fixture 纯校验通过，已用本机 `clang++` 对已知正确实现执行真实编译与测试，项目全量测试为 551 passed。尚未在个人电脑发送 live Eval 请求；正式三次基线留给配置公司 provider 和专用编译器适配器的公司电脑执行。
...
1. 代表性 C/C++ 仓库使用的构建系统、测试框架、测试命令，以及规格和规则文件的实际位置与格式。
2. 公司接口对多 tool calls、tool-result 回灌、structured output、上下文限制、取消和错误结构的支持程度。
3. 公司 GitLab 的大致版本、允许的 PAT scope 和使用政策；`glab` 是否可用是可选信息，不再作为方案前置条件。

## 18. 下一步

下一步优先完成阶段 2 的第一轮基线与质量闭环：

1. 在公司电脑运行三次 C 风格 `.cpp` 协议解析基线，并按失败类型形成第一份 Agent 质量报告；
2. 根据基线只选择一个最主要的质量问题进入第一轮 Agent 调优，然后用相同案例复测；
3. 为多文件分析和简单对话补充最小基线任务；
4. 扩展公司 Chat Completions conformance suite，覆盖尚未验证的工具回灌、结构化输出、上下文和错误契约；
5. 后续再处理本地安全边界、GitLab capability probe 和其他阶段。
