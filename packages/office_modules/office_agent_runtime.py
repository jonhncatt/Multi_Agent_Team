...
class OfficeAgent:
    ...
    def _successful_write_targets(self, tool_events: list[ToolEvent]) -> list[str]:
        return successful_write_targets_helper(self, tool_events)

    def _text_acknowledges_written_targets(self, text: str, targets: list[str]) -> bool:
        return text_acknowledges_written_targets_helper(text, targets)
    ...
    def _extract_citations_from_tool_result(
        ...
        *,
        ...
        return extract_citations_from_tool_result_helper(
            self,
            name=name,
            arguments=arguments,
            result=result,
    ...
    def _merge_citation_candidates(
        ...
        return merge_citation_candidates_helper(existing, incoming)

    def _finalize_citation_candidates(self, citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return finalize_citation_candidates_helper(self, citations)

    def _domain_from_url(self, raw_url: str) -> str | None:
        return domain_from_url_helper(raw_url)
    ...
    def build_session_route_state(self, route: dict[str, Any]) -> dict[str, Any]:
        return self._build_session_route_state(route)
    ...
    def normalize_route_decision(
        ...
        *,
        ...
        effective_fallback = dict(fallback or route or {})
        effective_settings = settings if settings is not None else ChatSettings()
        return self._normalize_route_decision_impl(
            route=route,
    ...
    def _router_signal_extractor(self) -> RouterSignalExtractor:
        return RouterSignalExtractor(
            self,
            news_hints=_NEWS_HINTS,
            followup_reference_hints=_FOLLOWUP_REFERENCE_HINTS,
            followup_transform_hints=_FOLLOWUP_TRANSFORM_HINTS,
        )
