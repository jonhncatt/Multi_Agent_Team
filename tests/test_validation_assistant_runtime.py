...
from app import validation_assistant_runtime as runtime_module
...
def test_stream_transaction_id_is_unique_when_provider_reuses_call_id() -> None:
    first = ValidationAssistantRuntime._typed_tool_item_id(
        run_id="run-1",
    ...
    second = ValidationAssistantRuntime._typed_tool_item_id(
        run_id="run-1",
        raw_tool_call={"id": "duplicate-call"},
        tool_name="read_file",
        round_idx=2,
    ...
    def execute(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        if name == "image_read":
            ...
            return {
                ...
                "width": 494,
                "height": 102,
                "visible_text": "Validation Assistant\nVA\nnew_validation_agent",
                ...
                "diagnostics": {
                    ...
                    "visible_text_preview": "Validation Assistant / VA / new_validation_agent",
                },
            }
        return super().execute(name, arguments)
...
def test_runtime_only_forwards_reasoning_effort_to_supported_models(
    ...
    supported_runtime = ValidationAssistantRuntime(
        config=_isolated_config(tmp_path),
        kernel_runtime=object(),
        agent_dir=agent_dir,
    ...
    locked_runtime = ValidationAssistantRuntime(
        config=_isolated_config(tmp_path),
        kernel_runtime=object(),
        agent_dir=agent_dir,
...
def test_runtime_finishes_as_cancelled_when_model_invocation_is_detached(tmp_path: Path) -> None:
    ...
    backend = _FakeBackend(
        [
            _FakeMessage(
                content="",
                additional_kwargs={"va_model_invocation_cancelled": True},
            )
        ]
    )
    runtime = ValidationAssistantRuntime(
...
def test_runtime_parses_frontmatter_and_prompt_order(tmp_path: Path) -> None:
    ...
    assert descriptor["agent_id"] == "validation_assistant"
...
def test_turn_changes_keep_file_updates_separate_from_tool_failures_and_latest_verification() -> None:
    ...
    changes = ValidationAssistantRuntime._build_turn_changes(events, turn_status="failed")
...
def test_turn_changes_do_not_claim_failed_or_skipped_patch_writes() -> None:
    ...
    changes = ValidationAssistantRuntime._build_turn_changes(events, turn_status="cancelled")
...
def test_agent_specs_define_v2_contract_and_tool_guidance() -> None:
    zh_spec_dir = REPO_ROOT / "agents" / "validation_assistant" / "locales" / "zh-CN"
...
def test_rendered_prompt_has_single_policy_owners() -> None:
    runtime = ValidationAssistantRuntime(
        config=load_config(),
        kernel_runtime=object(),
        agent_dir=REPO_ROOT / "agents" / "validation_assistant",
        backend=_FakeBackend([_FakeMessage(content="ok")]),
    )
...
def test_runtime_activity_helpers_use_requested_locale() -> None:
    assert ValidationAssistantRuntime._validation_activity_detail(
        "zh-CN",
        {"allowed": True, "tool_name": "read_file"},
    ) == translate("zh-CN", "runtime.activity.guard.normalized_continued", tool="read_file", suffix="")
    assert ValidationAssistantRuntime._execution_activity_detail("ja-JP", {}) == translate(
        "ja-JP",
        "runtime.activity.execution.recorded",
    )
    assert ValidationAssistantRuntime._validation_activity_detail(
...
def test_subagent_state_is_rebuilt_after_history_is_compacted(tmp_path: Path) -> None:
    ...
    runtime = ValidationAssistantRuntime(config=load_config(), kernel_runtime=object(), agent_dir=agent_dir, backend=backend)
...
def test_runtime_runs_single_agent_tool_loop(tmp_path: Path) -> None:
    ...
    assert result["agent_id"] == "validation_assistant"
...
def test_runtime_subagent_uses_isolated_read_only_context_and_returns_summary(
    ...
    monkeypatch.setattr(runtime_module, "create_va_runtime_backend", lambda _config: child_backend)
...
def test_runtime_runs_independent_subagents_in_parallel_and_waits_for_both(
    ...
    monkeypatch.setattr(runtime_module, "create_va_runtime_backend", create_child_backend)
...
def test_runtime_can_wait_for_and_recollect_subagent_from_an_earlier_agent_run(
    ...
    first_runtime = ValidationAssistantRuntime(
        config=config,
        kernel_runtime=object(),
        agent_dir=agent_dir,
    ...
    second_runtime = ValidationAssistantRuntime(
        config=config,
        kernel_runtime=object(),
        agent_dir=agent_dir,
    ...
    third_runtime = ValidationAssistantRuntime(
        config=config,
        kernel_runtime=object(),
        agent_dir=agent_dir,
...
def test_runtime_empty_parent_response_cancels_active_subagent_without_waiting(
    ...
    lookup_runtime = ValidationAssistantRuntime(
        config=_isolated_config(tmp_path),
        kernel_runtime=object(),
        agent_dir=agent_dir,
...
def test_tool_message_compaction_keeps_actionable_paths() -> None:
    ...
    compact = ValidationAssistantRuntime._compact_tool_result_for_model(result, tool_name="glob_file_search")
...
def test_tool_call_preview_preserves_uuid_identity_but_masks_arguments() -> None:
    ...
    preview = ValidationAssistantRuntime._safe_tool_call_preview(
        {
            "id": call_id,
            "name": "web_search",
    ...
    runtime.run(
        ...
        context={
            ...
            "project": {
                "project_root": str(tmp_path),
                "cwd": str(tmp_path),
                "profile_key": "builtin:validation-assistant",
...
def test_runtime_ignores_legacy_emergency_tool_call_cap(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    import app.validation_assistant_runtime as runtime_module
...
def test_runtime_does_not_auto_rescue_missing_context_reply_for_image_attachments(tmp_path: Path) -> None:
    ...
    backend = _FakeBackend(
        [
            ...
            _FakeMessage(content="图片里显示的是 Validation Assistant 的首页。"),
        ]
    )
    runtime = ValidationAssistantRuntime(
        config=load_config(),
...
def test_runtime_does_not_override_model_answer_after_image_read(tmp_path: Path) -> None:
    ...
    assert "Validation Assistant" in result["tool_events"][0]["diagnostics"]["visible_text_preview"]
...
def test_runtime_keeps_successful_tool_evidence_without_task_checkpoint(tmp_path: Path) -> None:
    ...
    backend = _FakeBackendWithTools(
        [
            _FakeMessage(content="", tool_calls=[{"id": "tc1", "name": "image_read", "args": {"path": str(image_path)}}]),
            _FakeMessage(content="图片里是 Validation Assistant 的首页"),
        ],
        _FakeImageReadTools(),
    )
    runtime = ValidationAssistantRuntime(
...
def test_runtime_initial_prompt_lists_skills_without_full_skill_body(tmp_path: Path) -> None:
    ...
    assert "VA_SKILL_ROOT" in available_section
    assert "VA_PROJECT_ROOT" in available_section
...
def test_runtime_boundary_makes_enabled_team_skill_writable_when_runtime_allows_writes(tmp_path: Path) -> None:
    ...
    available = runtime._enabled_skills("validation_assistant")
    ...
    runtime._extend_runtime_boundary_for_skills(
        disabled_boundary,
        runtime._enabled_skills("validation_assistant"),
    )
