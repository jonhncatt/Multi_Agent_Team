...
def run_revision_role(agent: Any, *, context: RoleContext) -> RoleResult:
    ...
    revision_system_prompt = (
        ...
        "禁止把最终答复改写成只有“结论不采用/调查无效”的一句话；"
        "即使证据不足，也必须保留已确认事实、未确认点和下一步核对建议。"
        "当某个技术原因尚未被完全证明（例如空指针触发链路、空函数成因），"
        "要写成“当前证据不足以确认原因”，而不是直接否决全部调查结果。"
        "如果现象与实现不一致，应明确提示“代码实现可能存在缺陷或与预期不一致”。"
