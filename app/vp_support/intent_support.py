...
def looks_like_understanding_request(
    ...
    *,
    ...
    if agent._requires_evidence_mode(user_message, []):
        return False
    tool_markers = (
        "read_file", "list_dir", "glob_file_search", "search_contents_in_file", "search_contents_in_file_multi",
...
def evidence_mode_needs_more_support(agent: Any, ai_msg: Any, tool_events: list[Any], spec_lookup_request: bool = False) -> bool:
    ...
    if spec_lookup_request and not ({"search_contents_in_file"} & tool_names):
        return True
    if not evidence_tool_hits:
        return True
    if spec_lookup_request and not (
        {"read_file", "read_section", "table_extract", "fact_check_file"} & tool_names
    ):
        return True
...
def request_likely_requires_tools(agent: Any, user_message: str, attachment_metas: list[dict[str, Any]], *, news_hints: tuple[str, ...]) -> bool:
    ...
    direct_hints = (
        ...
        "tests", "case", "在哪", "搜索", "上网", "网上", "查一下", "搜一下", "read_file", "list_dir", "glob_file_search", "search_contents_in_file",
        ...
        "shadow", "self-upgrade", "self upgrade", "self-update", "self update", "upgrade run", "patch_worker",
        "promote", "rollback",
    )
