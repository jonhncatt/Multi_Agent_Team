...
class SearchFilesArgs(BaseModel):
    query: str = Field(min_length=1, max_length=256, description="Fuzzy filename or path, e.g. varb or runtime backend.")
    ...
    refresh: bool = Field(default=False, description="Rebuild after external filesystem or ignore changes; VA mutations already invalidate snapshots lazily.")
...
class VARuntimeBackend:
    """Minimal app-owned backend for the active Validation Assistant runtime."""
    ...
    def _build_langchain_tools(self) -> list[Any]:
        return [
            ...
            self._StructuredTool.from_function(
                name="search_files",
                description="Fuzzy filename/path search using a reusable progressive snapshot. VA mutations invalidate automatically; check walk_complete and refresh after external changes. Does not search contents.",
            ...
            self._StructuredTool.from_function(
                name="save_skill",
                description="Create a repository-shared Team SKILL.md in the global VA Skill Registry, or replace it only when overwrite is true. Use apply_patch for Team Skill scripts, references, or partial edits; Built-in Skills are read-only.",
...
def classify_va_llm_exception(exc: BaseException, *, phase: str, model: str) -> dict[str, Any]:
