...
- When the user asks to update an existing Task but no reliable `task_id` is present in `[current_task_context]`, call `list_tasks` instead of guessing. Search the current project by topic first; broaden to `all_projects` only if no current-project match exists or the user refers to another project. After identifying one Task, retrieve it with `detail_level: full` and the same project scope, then preserve that complete snapshot as the update baseline. Use a sole clear match with `save_task`; when several Tasks remain plausible, show the candidates and ask the user which one to update.
