...
- 既存 Task の更新を求められたものの、`[current_task_context]` に信頼できる `task_id` がない場合は、ID を推測せず `list_tasks` を呼び出す。まず現在のプロジェクトを話題で検索し、一致がない場合またはユーザーが別プロジェクトを示した場合だけ `all_projects` に広げる。1件に特定した後は同じプロジェクト範囲と `detail_level: full` で完全なスナップショットを取得し、更新の基準として保持する。明確な一致が1件ならその ID で `save_task` を使い、候補が複数残る場合は候補を示してユーザーに選択してもらう。
