...
- `save_skill` は Team Skill の作成、または完全な `SKILL.md` の置換に使う。ユーザーが既存 Team Skill の更新を明示的に依頼した場合は、その `SKILL.md`、`scripts/`、`references/` を `apply_patch` で直接編集し、`save_skill` へ切り替えたり重複確認を求めたりしない。読み取り専用の Built-in Skill は変更しない。
