...
`evals/tool_failure_recovery_cases.json` runs focused Runtime continuation cases with fake tools and zero provider calls. It verifies that `where`/`rg` no-match results are not failures; repeated actions, failures, environment errors, and policy rejections are returned to the model without Runtime-imposed failure budgets, progress guesses, or forced replans; malformed native tool calls receive protocol repair; retained workspace changes stay independent from tool-failure history; cancellation reaches an authoritative terminal state before an immediate same-thread retry begins; an uncollected Subagent does not hold the parent Turn open and publishes its late result through the parent Thread mailbox; and output continuation and compaction remain bounded. It retains the original company regression in which `search_codebase` receives a file as its root, `select-string` is rejected, and the model then succeeds with `rg`.
